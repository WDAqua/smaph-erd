...
public class ExampleGatherer<T extends Object> {
	private Vector<List<Pair<FeaturePack<T>, Double>>> featureVectorsAndGoldGroups = new Vector<>();
	...
	public int getExamplesCount() {
		int count = 0;
		for (List<Pair<FeaturePack<T>, Double>> featureVectorAndGold : featureVectorsAndGoldGroups)
	...
	public List<FeaturePack<T>> getAllFeaturePacks() {
		...
		for (List<Pair<FeaturePack<T>, Double>> featureVectorsAndGoldGroup : featureVectorsAndGoldGroups)
	...
	private static <T extends FeaturePack> Vector<Pair<T, Double>> getPlain(
		...
		for (List<Pair<T, Double>> group : vectVect)
			for (Pair<T, Double> ftrVectAndGold : group) {
				res.add(new Pair<T, Double>(ftrVectAndGold.first,
	...
	public svm_problem generateLibSvmProblem(int[] pickedFtrs,
		...
		Vector<Pair<FeaturePack<T>, Double>> plainVectors = getPlain(featureVectorsAndGoldGroups);
	...
	public List<svm_problem> generateLibSvmProblemOnePerInstance(
		...
		for (List<Pair<FeaturePack<T>, Double>> ftrVectorsAndGolds : featureVectorsAndGoldGroups) {
			Vector<Double> targets = new Vector<Double>();
			Vector<svm_node[]> ftrVectors = new Vector<svm_node[]>();
	 ...
	 */
	public void dumpExamplesLibSvm(String filename, FeatureNormalizer fn)
			throws IOException {
		BufferedWriter wr = new BufferedWriter(new FileWriter(filename, false));

		for (int i = 0; i < featureVectorsAndGoldGroups.size(); i++)
			for (Pair<FeaturePack<T>, Double> vectAndGold : featureVectorsAndGoldGroups
					.get(i))
				writeLineLibSvm(fn.ftrToNormalizedFtrArray(vectAndGold.first),
						wr, vectAndGold.second, i);
		;
		wr.close();
	}

	private void writeLineLibSvm(double[] ftrVect, BufferedWriter wr,
			double gold, int id) throws IOException {
	...
	public void dumpExamplesRankLib(String filename, FeatureNormalizer fn)
		...
		for (int groupId = 0; groupId < featureVectorsAndGoldGroups.size(); groupId++) {
			List<Pair<FeaturePack<T>, Double>> featureVectorsAndGolds = new Vector<>(
					featureVectorsAndGoldGroups.get(groupId));
